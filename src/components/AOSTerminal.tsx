...
export function AOSTerminal({ sessionProcessId }: AOSTerminalProps) {
  ...
  useEffect(() => {
    if (terminalRef.current && terminal == null && !isTerminalInitialized) {
      ...
      newTerminal.onData((data) => {
        ...
        if (
          data.length > 1 ||
          (data.charCodeAt(0) < 32 && data.charCodeAt(0) !== 13)
        ) {
          console.log(
            'Key data:',
        ...
        for (let i = 0; i < data.length; i++) {
          ...
          if (code === 13) {
            ...
            if (i + 2 < data.length && data[i + 1] === '[') {
              ...
              if (escapeCode === 'A') {
                ...
                if (historyPos < commandHistory.length - 1) {
                  historyPos++
                  const cmd =
                    commandHistory[commandHistory.length - 1 - historyPos]
                  currentInput = cmd || ''
            ...
            } else if (i + 3 < data.length && data[i + 1] === 'b') {
              // Alt+Left (word left)
              ...
              i += 1 // Skip the 'b'
            } else if (i + 3 < data.length && data[i + 1] === 'f') {
              ...
              i += 1 // Skip the 'f'
            } else if (i + 1 < data.length && data[i + 1] === '\x7f') {
              // Alt+Backspace (delete word backward)
              if (cursorPosition > 0) {
                const wordStart = findWordStart(currentInput, cursorPosition)
                currentInput =
                  currentInput.slice(0, wordStart) +
                  currentInput.slice(cursorPosition)
                cursorPosition = wordStart
                renderLine()
              }
              ...
              if (cursorPosition < currentInput.length) {
                ...
                currentInput =
                  currentInput.slice(0, cursorPosition) +
                  currentInput.slice(wordEnd)
                renderLine()
              }
              i += 5 // Skip the entire sequence
            } else if (
              i + 4 < data.length &&
              data.slice(i + 1, i + 5) === '[1;3'
            ) {
              // Alt modifier sequences
              if (i + 5 < data.length && data[i + 5] === 'D') {
                // Alt+Left
                const wordStart = findWordStart(currentInput, cursorPosition)
                const moves = cursorPosition - wordStart
                cursorPosition = wordStart
                ...
                i += 5
              } else if (i + 5 < data.length && data[i + 5] === 'C') {
                // Alt+Right
                const wordEnd = findWordEnd(currentInput, cursorPosition)
                const moves = wordEnd - cursorPosition
                cursorPosition = wordEnd
  ...
  return (
    <div className="flex h-full flex-col">
      {/* Connection Status */}
      {error && (
        <div className="border-destructive/20 bg-destructive/10 m-2 rounded-lg border p-3 text-sm text-destructive">
          Error: {error}
        </div>
      )}
      ...
      {!sessionProcessId && (
        <div className="m-2 rounded-lg border border-yellow-200 bg-yellow-50 p-3 text-sm text-yellow-800 dark:border-yellow-800 dark:bg-yellow-950 dark:text-yellow-200">
          No process ID configured. Use the settings button to configure this
          terminal session.
        </div>
      )}
        ...
        {!sessionProcessId && (
          <div className="absolute inset-0 flex items-center justify-center text-muted-foreground">
            <div className="text-center">
              ...
              <p className="text-sm">
                Configure a process ID in the session settings
              </p>
            </div>
          </div>
        )}
