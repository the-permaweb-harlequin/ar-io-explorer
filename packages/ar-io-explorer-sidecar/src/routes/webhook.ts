import { z } from 'zod';
import type { Transaction, Block, ApiResponse } from '@the-permaweb-harlequin/shared-types';
...
export function webhookRoutes(router: any, parquetManager: ParquetManager, logger: Logger) {
  ...
  router.post('/webhook/transaction', async (ctx) => {
    try {
      ...
      logger.info(`📨 Processing transaction: ${transaction.transaction_id} (block: ${transaction.block_height})`);
  ...
  router.post('/webhook/block', async (ctx) => {
    try {
      ...
      logger.info(`📦 Processing block: ${block.block_height} with ${block.transactions?.length || 0} transactions`);
      ...
      if (block.transactions && block.transactions.length > 0) {
        for (const tx of block.transactions) {
          try {
            ...
            logger.error(txError, `Failed to process transaction ${tx.transaction_id} in block ${block.block_height}`);
            errorCount++;
          }
        }
      }
