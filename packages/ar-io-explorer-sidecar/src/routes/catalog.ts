    ...
    try {
      ...
      const validation = {
        ...
        sufficient_balance: balance
          ? parseFloat(balance.balance) > 0.01
          : false, // Minimum 0.01 AR
        ready_for_deployment: false,
      }

      validation.ready_for_deployment =
        validation.wallet_configured &&
      ...
      ctx.body = {
        ...
        response: {
          ...
          recommendations: validation.ready_for_deployment
            ...
            : [
                !validation.wallet_configured
                  ? 'Configure wallet at ARWEAVE_WALLET_PATH'
                  : null,
                !validation.arweave_connection
                  ? 'Check Arweave connection settings'
                  : null,
                !validation.sufficient_balance
                  ? 'Insufficient AR balance for deployment'
                  : null,
              ].filter(Boolean),
        },
      }
