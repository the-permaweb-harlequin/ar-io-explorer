...
import type { CatalogInfo, DataCatalog, TableInfo, WalletBalance } from '@the-permaweb-harlequin/shared-types';
...
export class CatalogManager {
  ...
  async init(): Promise<void> {
    ...
    try {
      ...
      console.warn('⚠️  No wallet found, checkpoint deployment will be disabled');
  ...
  async createCheckpoint(parquetFiles: Array<string>): Promise<string> {
    ...
    const uploadPromises = parquetFiles.map(filePath => this.uploadParquetFile(filePath));
  ...
  async uploadParquetFile(filePath: string): Promise<string> {
    ...
    console.log(`📤 Uploading ${tableName} to Arweave (${fileData.length} bytes)...`);
  ...
  async uploadCatalog(catalog: DataCatalog): Promise<string> {
    ...
    console.log(`📤 Uploading catalog to Arweave (${catalogData.length} bytes)...`);
    ...
    transaction.addTag('Table-Count', Object.keys(catalog.tables).length.toString());
  ...
  async getCatalogInfo(): Promise<CatalogInfo> {
    return {
      ...
      wallet_address: this.wallet ? await this.arweave.wallets.jwkToAddress(this.wallet) : null,
