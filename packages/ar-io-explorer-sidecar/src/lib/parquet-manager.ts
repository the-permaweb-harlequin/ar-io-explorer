...
export class ParquetManager {
  ...
  async init(): Promise<void> {
    ...
    console.log(`📊 ParquetManager initialized with batch size: ${this.batchSize}`);
  }

  setupSchemas(): void {
    // ArNS Names schema
    this.schemas.set('arns_names', new ParquetSchema({
      name: { type: 'UTF8' },
      owner: { type: 'UTF8' },
      target: { type: 'UTF8' },
    ...
    // Transactions schema
    this.schemas.set('transactions', new ParquetSchema({
      id: { type: 'UTF8' },
      owner: { type: 'UTF8' },
    ...
    // ANT Registry schema
    this.schemas.set('ant_registry', new ParquetSchema({
    ...
    // ArNS Records schema
    this.schemas.set('arns_records', new ParquetSchema({
      name: { type: 'UTF8' },
    ...
    // AO Processes schema
    this.schemas.set('ao_processes', new ParquetSchema({
      process_id: { type: 'UTF8' },
      owner: { type: 'UTF8' },
      module_id: { type: 'UTF8' },
      scheduler: { type: 'UTF8' },
    ...
    // AO Messages schema
    this.schemas.set('ao_messages', new ParquetSchema({
  ...
  async processTransaction(transaction: Transaction): Promise<void> {
    const { transaction_id, owner, target, tags, data_size, block_height, block_timestamp, fee } = transaction;
  ...
  classifyTransaction(tags: Array<Tag>): TransactionClassification {
    ...
    if (tagMap.has('Data-Protocol') && tagMap.get('Data-Protocol') === 'ao' && tagMap.has('Type') && tagMap.get('Type') === 'Process') {
      return {
        type: 'ao_process',
        data: {
    ...
    if (tagMap.has('Data-Protocol') && tagMap.get('Data-Protocol') === 'ao' && tagMap.has('Type') && tagMap.get('Type') === 'Message') {
      return {
        type: 'ao_message',
        data: {
  ...
  async flushTable(tableName: string): Promise<void> {
    ...
    try {
      ...
      console.log(`✅ Flushed ${buffer.length} records to ${tableName}.parquet (total: ${allData.length})`);
    } catch (error) {
  ...
  async queryTable(tableName: string, query: string, params: Array<any> = []): Promise<Array<any>> {
    const filePath = path.join(this.basePath, `${tableName}.parquet`);
